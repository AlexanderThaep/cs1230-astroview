...
void MainWindow::initialize(const std::string &sceneFilePath)
{
    ...
    farSlider->setMinimum(1);
    farSlider->setMaximum(1000);
    farSlider->setValue(10);

    farBox = new QDoubleSpinBox();
    farBox->setMinimum(1.0f);
    farBox->setMaximum(10.f);
    ...
    // Extra Credit:
    ec1 = new QCheckBox();
    ec1->setText(QStringLiteral("Adaptive LOD"));
    ...
    onValChangeFarBox(1.0f);

    settings.sceneFilePath = sceneFilePath;
}
...
void MainWindow::onValChangeNearSlider(int newValue)
{
    ...
    realtime->CameraSettingsChanged();
}
